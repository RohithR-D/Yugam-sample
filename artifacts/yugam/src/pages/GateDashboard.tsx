...
function GateDashboardView() {
  const [metrics, setMetrics] = useState({ currentOccupancy: 0, totalToday: 0, expectedVIPs: 0 });
  ...
  const fetchData = useCallback(async () => {
    try {
      const [dashRes, visitorsRes] = await Promise.all([
        authFetch("/api/gate/dashboard"),
        authFetch("/api/gate/visitors?limit=10"),
      ]);
  ...
  const handleRollCall = async () => {
    try {
      const res = await authFetch("/api/gate/roll-call");
      if (res.ok) {
        setRollCallData(await res.json());
