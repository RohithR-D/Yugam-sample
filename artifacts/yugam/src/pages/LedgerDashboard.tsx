...
type LedgerSub = "Finance Dashboard" | "Chart of Accounts" | "Accounts Payable (AP)" | "Accounts Receivable (AR)" | "Journal Entries" | "Financial Statements";

interface CoaRecord { id: number; accountCode: string; accountName: string; accountType: string; currentBalance: string; parentId: number | null; description: string; isActive: string; createdAt: string | null; }
interface JournalEntry { id: number; entryDate: string; reference: string; description: string; totalDebit: string; totalCredit: string; status: string; createdAt: string | null; }
interface JournalLine { id: number; journalEntryId: number; accountId: number; accountCode: string; accountName: string; debit: string; credit: string; memo: string; }
interface APRecord { id: number; vendorName: string; billNumber: string; billDate: string; dueDate: string; amount: string; paidAmount: string; status: string; entryType: string; notes: string; createdAt: string | null; }
interface ARRecord { id: number; clientName: string; invoiceNumber: string; invoiceDate: string; dueDate: string; amount: string; receivedAmount: string; status: string; entryType: string; notes: string; createdAt: string | null; }
interface DashSummary { totalCash: number; totalReceivables: number; totalPayables: number; netIncome: number; arAging: { days30: number; days60: number; days90: number }; apAging: { days30: number; days60: number; days90: number }; }
...
export default function LedgerDashboard() {
  ...
  const sub = (activeModule.replace("Ledger:", "") || "Finance Dashboard") as LedgerSub;
  ...
  const fetchAll = useCallback(async () => {
    try {
      const [cR, jR, apR, arR, sR] = await Promise.all([
        authFetch("/api/ledger/coa"), authFetch("/api/ledger/journal-entries"),
        authFetch("/api/ledger/ap"), authFetch("/api/ledger/ar"),
        authFetch("/api/ledger/dashboard-summary"),
      ]);
      if (cR.ok) setCoa(await cR.json());
      if (jR.ok) setJournals(await jR.json());
      if (apR.ok) setAp(await apR.json());
      if (arR.ok) setAr(await arR.json());
