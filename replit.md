...
- Routes: `src/routes/index.ts` mounts sub-routers; `src/routes/health.ts` exposes `GET /healthz`; `src/routes/users.ts` exposes `GET /api/users` and `POST /api/users`
...
- **Full-stack modules**: Gate Security module fetches users from the database via `/api/users`
