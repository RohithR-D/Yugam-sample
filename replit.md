...
- **Estimo CPQ Engine:** Full Configure-Price-Quote system replacing the simple quotes list. Schema: `service_catalog` (template_name, description, tags, base_hours, base_rate), `proposals` (client_id FK, title, status, total_estimated_hours, grand_total, proposal_data JSONB). API: CRUD `/api/service-catalog`, full CRUD `/api/proposals` (GET list with client join, GET by id, POST, PATCH with field validation, DELETE). Frontend: 3-tab layout (Proposals table, Service Catalog grid, Analytics dashboard). ProposalBuilder: hierarchical Category → Feature cost structure with inline editing, dynamic hour/rate/total calculations, client linking, section navigation, and real-time summary panel. PATCH validation enforces status enum, non-negative numerics, and array shape for proposalData.
